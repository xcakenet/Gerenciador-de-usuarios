...
const Dashboard: React.FC<DashboardProps> = ({ users, systems }) => {
  ...
  const chartData = useMemo(() => {
    if (systems.length > 0) {
      return systems.map(s => ({ name: s.name, count: s.userCount }));
    }
    return [{ name: 'Aguardando Dados', count: 0 }];
  }, [systems]);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
          ...
          {/* Container com altura mínima fixa para evitar erro de cálculo do Recharts */}
          ...
          <div className="flex-1 space-y-4 overflow-y-auto max-h-[350px] pr-2 custom-scrollbar">
            {localInsights.length === 0 ? (
              ...
              localInsights.map((insight, idx) => (
                <div key={idx} className={`p-4 rounded-xl border flex items-start gap-3 transition-all hover:scale-[1.02] ${
